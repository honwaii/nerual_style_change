...
class Model(object):
    ...
    def vggnet(self):
        # 读取预训练的vgg模型
        vgg = scipy.io.loadmat(settings.VGG_MODEL_PATH)  # vgg的类型是字典：key包含__header__,__version__,__globals__,layers,classes, normalization
        vgg_layers = vgg['layers'][0]
        net = {}
        # 使用预训练的模型参数构建vgg网络的卷积层和池化层
        # 全连接层不需要
        # 注意，除了input之外，这里参数都为constant，即常量,和平时不同，我们并不训练vgg的参数，它们保持不变,需要进行训练的是input，它即是我们最终生成的图像
